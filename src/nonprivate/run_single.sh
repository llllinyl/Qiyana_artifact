...
echo "============================================="
echo "🔧  Client-to-Server Nonprivate Test (With Logs)"
echo "============================================="

rm -f ./result/result_single_server.txt ./result/result_single_client.txt 2>/dev/null

pkill -f "baseline_server\|baseline_client" 2>/dev/null || true

echo -e "\n1. Starting Server (logs to ./result/result_single_server.txt)..."
RUSTFLAGS="-C target-cpu=native" cargo run --release --bin nonprivate_server > ./result/result_single_server.txt 2>&1 &
SERVER_PID=$!
...
echo "================ CLIENT OUTPUT ================"
RUSTFLAGS="-C target-cpu=native" cargo run --release --bin nonprivate_client 2>&1 | tee ./result/result_single_client.txt
CLIENT_EXIT=$?
echo "================ CLIENT ENDED ================="
