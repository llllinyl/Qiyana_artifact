...
impl MasterNode {
        ...
        match bincode::deserialize::<Vec<Vec<u8>>>(&all_data) {
            Ok(params_pack) => {
                ...
                if params_pack.len() != 7 {
                    println!("[{}] ❌ Expected 7 parameters, got {}", client_addr, params_pack.len());
                    return;
                }
                ...
                match bincode::deserialize::<Vec<Vec<u8>>>(&query_data) {
                    Ok(query_vec) => {
                        ...
                        if query_vec.len() != 3 {
                            println!("[{}] ❌ Expected 3 query element, got {}", client_addr, params_pack.len());
                            return;
                        }
    ...
    async fn distribute_public_parameters_to_workers(&self, params_pack: Vec<Vec<u8>>) {
        ...
        for (&worker_id, stream) in workers.iter_mut() {
            println!("   Sending to Worker {}...", worker_id);
