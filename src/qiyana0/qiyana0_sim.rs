...
impl Client {
    ...
    pub fn qiyana0_packing_recovery(&self, results: Vec<GlweCiphertext<Vec<u64>>>) -> Vec<u16> {
        let message_modulus = 1u64 << 7;
        let delta = (1_u64 << 63) / message_modulus;
...
#[test]
fn test_qiyana0_simulate(){
    rayon::ThreadPoolBuilder::new()
        .num_threads(THREAD_NUM)
        .build_global()
        .unwrap();
    let keyword_path = "/root/qiyana0-experiment/keyword.txt";
...
#[test]
fn test_qiyana0_zip_simulate(){
    rayon::ThreadPoolBuilder::new()
        .num_threads(THREAD_NUM)
        .build_global()
        .unwrap();
    let keyword_path = "/home/lyl/Desktop/qiyana0/keyword.txt";
    let tfidf_path = "/home/lyl/Desktop/qiyana0/tf-idf.txt";
    let client = Client::new();
    let pre = Instant::now();
    let server = Server::new(client.seeded_bsk.clone(), 
        client.seeded_ksk.clone(), 
