...
use std::io::*;
include!("baselineh_sim.rs");

#[tokio::main]
async fn main() {
    println!("========================================");
    println!("🔍 BaselineH Client");
    println!("========================================\n");
    ...
    let length = HASH_FHE_STRING_LEN;
    ...
    let (query, format) = client.baselineh_query(test_string, length);
    ...
    match bincode::deserialize::<Vec<FheBool>>(&all_response_data) {
        Ok(response) => {
            ...
            let recovered = client.baselineh_recovery(response);
