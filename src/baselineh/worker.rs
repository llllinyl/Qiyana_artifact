    ...
    match bincode::deserialize::<Vec<Vec<u8>>>(&all_data) {
        Ok(params_pack) => {
            ...
            if params_pack.len() != 2 {
                println!("[{}] ❌ Expected 2 parameters, got {}", master_addr, params_pack.len());
                return;
            }
            ...
            let server_key: ServerKey = match bincode::deserialize(&params_pack[0]) {
                ...
                Err(e) => {
                    println!("[{}] ❌ Failed to deserialize server_key: {}", master_addr, e);
                    return;
                }
            };
            ...
            let false_ciphertext: FheBool = match bincode::deserialize(&params_pack[1]) {
                ...
                Err(e) => {
                    println!("[{}] ❌ Failed to deserialize false_ciphertext: {}", master_addr, e);
                    return;
                }
            };
            ...
            match bincode::deserialize::<Vec<Vec<u8>>>(&query_data) {
                Ok(query_vec) => {
                    ...
                    if query_vec.len() != 2 {
                        println!("[{}] ❌ Expected 2 query element, got {}", master_addr, params_pack.len());
                        return;
                    }
                    let deserialize_start = Instant::now();
                    ...
                    let format: Vec<QueryElement> = match bincode::deserialize(&query_vec[1]) {
                        ...
                        Err(e) => {
                            println!("[{}] ❌ Failed to deserialize format: {}", master_addr, e);
                            return;
                        }
                    };
                    ...
                    rayon::ThreadPoolBuilder::new()
                        ...
                        .build_global()
                        .unwrap();
                    let response = server.baselineh_response(query, &format);
...
async fn send_response_to_client(worker_id: u32, response: Vec<FheBool>) {
    let client_addr = format!("127.0.0.1:{}", CLIENT_RESULT_PORT);
