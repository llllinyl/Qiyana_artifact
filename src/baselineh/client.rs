...
async fn main() {
    ...
    let (encrypted_results, queryformat) = client.baselineh_query(
        &test_string,
        HASH_FHE_STRING_LEN,
    );
    ...
    let query_vec = vec![
        bincode::serialize(&encrypted_results).unwrap(),
        bincode::serialize(&queryformat).unwrap(),
    ];
...
async fn receive_and_process_results(client: Client, listener: TcpListener, expected_workers: usize) {
    ...
    let recovered = client.baselineh_recovery(merged_and_sorted);
