...
pub const HASH_OUTPUT_BITS: usize = 70;
pub const HASH_SOURCE_BYTES: usize = HASH_OUTPUT_BITS.div_ceil(8);
pub const HASH_STRING_LEN: usize = 11;
pub const HASH_FHE_STRING_LEN: usize = HASH_STRING_LEN;

pub fn hash_keyword_to_string(keyword: &str) -> String {
    let digest = Sha256::digest(keyword.as_bytes());
    let mut value = 0u128;
    for byte in digest.iter().take(HASH_SOURCE_BYTES) {
        value = (value << 8) | u128::from(*byte);
    }
    value >>= HASH_SOURCE_BYTES * 8 - HASH_OUTPUT_BITS;

    let mut encoded = [0u8; HASH_STRING_LEN];
    for byte in encoded.iter_mut().rev() {
        *byte = (value % 95) as u8 + b' ';
        value /= 95;
    }
    debug_assert_eq!(value, 0);

    String::from_utf8(encoded.to_vec()).expect("hash encoding must be ASCII")
}
...
impl Client {
    ...
    pub fn baselineh_query(&self, query_string: &str, length: usize) -> (Vec<FheAsciiString>, Vec<QueryElement>) {
        assert_eq!(length, HASH_FHE_STRING_LEN);
        let (strings, queryformat) = parse_query_to_rpn(query_string);

        let mut encrypted_results = Vec::with_capacity(strings.len());
        for string in strings {
            let hashed_string = hash_keyword_to_string(&string);
            let encrypted_string = FheAsciiString::try_encrypt_with_fixed_sized(
                &hashed_string,
                length,
                &self.client_key,
            ).unwrap();
...
impl SubServer {
        ...
        for doc_idx in start_doc..end_doc {
            ...
            for part in line.split(',') {
                ...
                if !trimmed.is_empty() && index < KEYWORD_SET_NUM {
                    keys[index] = hash_keyword_to_string(trimmed);
                    index += 1;
                }
            }
    ...
    pub fn baselineh_response(&self, query: Vec<FheAsciiString>,
