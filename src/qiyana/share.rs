    ...
    pub fn qiyana_packing_recovery(&self, results: Vec<GlweCiphertext<Vec<u64>>>, worker_num: usize) -> Vec<u16> {
        ...
        for num in 0..worker_num {
            for idx in 0..batches {
                let packed = results[num * batches + idx].clone();
...
impl SubServer {
    ...
    pub fn qiyana_response(&self, query: Vec<Vec<LweCiphertextOwned<u64>>>,
        ...
        let results: Vec<Vec<LweCiphertextOwned<u64>>> = (0..datalength)
            .into_par_iter()
            .map(|doc_idx| {
                (0..5)
                    .into_par_iter()
        ...
        let final_results: Vec<Vec<LweCiphertextOwned<u64>>> = (0..datalength)
            .into_par_iter()
            .map(|num| {
    ...
    pub fn qiyana_decompress_response(&self, query: Vec<Vec<LweCiphertextOwned<u64>>>,
        ...
        let all_containers: Vec<Vec<u64>> = {
            let flat_items: Vec<u64> = (0..datalength)
                .into_par_iter()
                .flat_map(|num| {
                    let row = results[num].clone();
                    let mask = match_results[num].clone();
