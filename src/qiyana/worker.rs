...
async fn main() {
    ...
    let mode = match args[3].parse::<usize>() {
        Ok(num) => num,
        Err(_) => {
            println!("Usage: <command> -- <worker_id> <worker_num> <MODE>");
            println!("Example: <command> -- 0 2 1");
    ...
    match bincode::deserialize::<Vec<Vec<u8>>>(&all_data) {
        Ok(params_pack) => {
            ...
            if params_pack.len() != 9 {
                println!("[{}] ❌ Expected 9 parameters, got {}", master_addr, params_pack.len());
                return;
            }
            ...
            match bincode::deserialize::<Vec<Vec<u8>>>(&query_data) {
                Ok(query_vec) => {
                    ...
                    if query_vec.len() != 4 {
                        println!("[{}] ❌ Expected 4 query element, got {}", master_addr, params_pack.len());
                        return;
                    }
                    let deserialize_start = Instant::now();
                    ...
                    if mode == 0 {
                        ...
                        let client_addr = format!("127.0.0.1:{}", CLIENT_RESULT_PORT);
                        println!("[Worker {}] 5. Connecting and sending to Client at {}...", worker_id, client_addr);
                        send_packed_response_to_client(worker_id, response).await;
                        println!("   ✅ Task spawned for Worker {}", worker_id);  
                    }
                }
                Err(e) => {
...
async fn send_response_to_client(worker_id: u32, response: Vec<Vec<LweCiphertextOwned<u64>>>) {
    let client_addr = format!("127.0.0.1:{}", CLIENT_RESULT_PORT);
...
async fn send_packed_response_to_client(worker_id: u32, response: Vec<GlweCiphertext<Vec<u64>>>) {
    let client_addr = format!("127.0.0.1:{}", CLIENT_RESULT_PORT);
