...
# (A AND NOT B) OR (C AND D AND NOT E) OR (F AND G AND H)
config = {
    "font.family": 'serif',
...
baseline2  = np.array([42553.67, 169961.27, 673614.98, 2693136.26]) 
qiyana2   = np.array([1461.35, 6370.75, 28116.39, 110069.21]) 
qiyanawosel2   = np.array([871.62, 3543.06, 14324.72, 55468.16]) 
...
p2 = ax2.bar(x_coeus, coeus2, bar_width, bottom=coeus1,
            label='Coeus-CPU2', color=c_coeus, edgecolor='black', linewidth=0.5, hatch='-')
...
p4 = ax2.bar(x_baseline, baseline2, bar_width, bottom=baseline1,
            label='Baseline-CPU2', color=c_baseline, edgecolor='black', linewidth=0.5, hatch='-')
...
p6 = ax2.bar(x_qiyana, qiyana2, bar_width, bottom=qiyana1,
            label='Qiyana-CPU2', color=c_qiyana, edgecolor='black', linewidth=0.5, hatch='-')
...
p8 = ax2.bar(x_qiyanawosel, qiyanawosel2, bar_width, bottom=qiyanawosel1,
            label='Qiyana-wosel-CPU2', color=c_qiyanawosel, edgecolor='black', linewidth=0.5, hatch='-')
...
for i in range(len(x_pos)):
    ...
    ax2.text(x_qiyana[i] + 0.08, total_qiyana * 1.2, f'{ratio_qiyana:.1f}' + r'$\times$', 
             ha='center', va='bottom', fontsize=10, family='Times New Roman',fontweight='bold')
    
    total_qiyanawosel = qiyanawosel1[i] + qiyanawosel2[i]
    ratio_qiyanawosel = max_val / total_qiyanawosel
    ax2.text(x_qiyanawosel[i] + 0.10, total_qiyanawosel * 1, f'{ratio_qiyanawosel:.1f}' + r'$\times$', 
             ha='center', va='bottom', fontsize=10, family='Times New Roman',fontweight='bold')
...
ax1.set_yticklabels(['$10$', '$10^2$', '$10^3$', '$10^4$', '$10^5$', '$10^6$'], fontsize=8)
...
ax2.tick_params(axis="x", which="major", direction="in", width=1, length=5, labelsize=10)
...
fig.text(0.52, 0.02, "Dataset Size " + r'$(|\mathcal{D}|)$', ha='center', fontsize=11, family='Times New Roman')
ax1.set_ylabel('Latency (s)', fontsize=12, family='Times New Roman', labelpad=0)
