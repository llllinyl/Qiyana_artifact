        ...
        Ok(params_pack) => {
            ...
            match bincode::deserialize::<Vec<Vec<u8>>>(&query_data) {
                Ok(query_vec) => {
                    ...
                    let query: Vec<EncryptedKeywordBloom> = match bincode::deserialize(&query_vec[0]) {
                        Ok(param) => {
                            param
                        }
                        Err(e) => {
                    ...
                    rayon::ThreadPoolBuilder::new()
                        ...
                        .build_global()
                        .unwrap();                 
                    let response = server.baselineb_response(query, &format);
