...
use std::io::*;
include!("baselineb_sim.rs");

#[tokio::main]
async fn main() {
    println!("========================================");
    println!("🔍 BaselineB Client");
    println!("========================================\n");
    ...
    let length = DEFAULT_SIZE as usize;
    ...
    let (query, format) = client.baselineb_query(test_string, length);
    ...
    match bincode::deserialize::<Vec<FheBool>>(&all_response_data) {
        Ok(response) => {
            ...
            let recovered = client.baselineb_recovery(response);
